...
public class BTreeSMGetOperationOldCallback<T>
      ...
      if (gp.mergedResult.size() == 0) {
        ...
        for (SMGetElement<T> result : eachResult) {
          for (; pos < gp.mergedResult.size(); pos++) {
            if (reverse && 0 < result.compareTo(gp.mergedResult.get(pos))) {
              break;
            } else if (0 > result.compareTo(gp.mergedResult.get(pos))) {
              break;
            }
          }
  ...
  public static class GlobalParams<T>
    ...
    private final AtomicBoolean mergedTrim;

    public GlobalParams(final CountDownLatch blatch,
                        ...
                        final AtomicInteger processedSMGetCount,
                        final List<AtomicBoolean> atomicBooleans) {
      super(blatch, lock, missedKeyList, missedKeys, mergedResult, mergedTrimmedKeys,
          resultOperationStatus, failedOperationStatus, processedSMGetCount, atomicBooleans);

      this.mergedTrim = atomicBooleans.get(1);
    }
  }
}
